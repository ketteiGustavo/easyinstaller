...
PACKAGE_MANAGERS = {
    # Debian/Ubuntu based
    "ubuntu": {"install": "sudo apt install -y", "remove": "sudo apt remove -y"},
    "debian": {"install": "sudo apt install -y", "remove": "sudo apt remove -y"},
    "linuxmint": {"install": "sudo apt install -y", "remove": "sudo apt remove -y"},
    # Arch based
    "arch": {"install": "sudo pacman -S --noconfirm", "remove": "sudo pacman -Rns --noconfirm"},
    "manjaro": {"install": "sudo pacman -S --noconfirm", "remove": "sudo pacman -Rns --noconfirm"},
    # Fedora/CentOS/RHEL based
    "fedora": {"install": "sudo dnf install -y", "remove": "sudo dnf remove -y"},
}
...
def install_package(package_name: str):
    ...
    if not commands:
        console.print(f"[red]Error:[/red] Unsupported distribution: {get_distro_id()}")
        raise typer.Exit(1)

    command = f"{commands['install']} {package_name}"
    console.print(f"[bold green]>>>[/bold green] Pretending to run: '[cyan]{command}[/cyan]'")
...
def remove_package(package_name: str):
    ...
    console.print(f"[bold red]>>>[/bold red] Pretending to run: '[cyan]{command}[/cyan]'")
...
def install_with_manager(package_name: str, manager: str):
    ...
    if manager == "flatpak":
        ...
        if commands and manager in commands['install']:
            ...
            console.print(f"[red]Error:[/red] Unsupported or unknown manager: {manager}")
            raise typer.Exit(1)
